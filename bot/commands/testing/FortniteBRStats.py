import discord
from discord import app_commands
from discord.ext import commands
import logging
import aiohttp

# Initialize logging
logger = logging.getLogger('FortniteBRStats.py')
logger.setLevel(logging.DEBUG)
handler = logging.FileHandler(filename='./logs/FortniteBRStats.py.log', encoding='utf-8', mode='w')
handler.setFormatter(logging.Formatter('%(asctime)s:%(levelname)s:%(name)s: %(message)s'))
logger.addHandler(handler)
logger.propagate = False
logger.info("Fortnite BR Stats Cog Loaded. Logging started...")

class FortniteBRStats(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot
        self.session = aiohttp.ClientSession()
    
    group = app_commands.Group(name="fortnite", description="Commands related to Fortnite.")

    @app_commands.command(name="stats", description="Get Fortnite BR stats for a player.")
    async def br_stats(self, interaction: discord.Interaction, username: str, account_type: str = 'epic', time_window: str = 'lifetime'):
        """ Fetches Fortnite BR player stats. """
        api_url = f"https://fortnite-api.com/v2/stats/br/v2"
        headers = {
            "Authorization": "a571032c-b62d-4c76-9448-d5a521a06e0e"  # Replace with your actual API key
        }
        params = {
            "name": username,
            "accountType": account_type,
            "timeWindow": time_window
        }

        async with self.session.get(api_url, headers=headers, params=params) as resp:
            if resp.status == 200:
                data = await resp.json()
                # Here you might want to format the data to make it more readable in Discord
                await interaction.response.send_message(f"Player BR Stats: {data}")
                logger.info(f"Fortnite BR stats retrieved for {username}.")
            else:
                await interaction.response.send_message("Failed to retrieve BR stats.")
                logger.error(f"Failed to retrieve Fortnite BR stats for {username}.")

    async def cog_unload(self):
        await self.session.close()  # Properly close the session when the cog is unloaded
    
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(FortniteBRStats(bot))
